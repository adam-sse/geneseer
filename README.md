...
| `--config.llm.api`                        | not set              | The API endpoint of the LLM. Consists of the provider type and the full endpoint URL, seperated by `+` (e.g. `ollama+http://localhost:11434/api/chat`). Valid providers are `ollama` and `openai`. The URL should end with `/api/chat` for `ollama` and `/v1/chat/completions` for `openai`. |
